...
class Org_TreeView extends RepoTreeView
{
	...
	function _getPropertiesId()
	{
		return 'org_opproperties';
	}
	function _getCategorizeId()
	{
		return 'org_opcategorize';
	}
	function _getAccessId()
	{
		return 'org_opaccess';
	}
	function _getOpUpId()
	...
	function _getOpDownId()
	{
		return '_orgopdown_';
	}
	function _getShowResultsId()
	{
	...
	function extendedParsing($arrayState, $arrayExpand, $arrayCompress)
	{
		...
		if (isset($arrayState['org_properties_ok'])) {
			...
			if ($arrayState['prerequisites'] != '' && $arrayState['prerequisites']{
				0} == ',') {
				...
				switch ($this->kind) {
					...
					default:
						if (checkPerm('lesson', true, 'storage') && !$this->playOnly) {
							...
							$out .= '<div class="has-forma-tooltip" style="float:right;">
