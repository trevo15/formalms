...
class Report_User extends Report {
	...
	function getLOTypesTranslations() {
		if (!is_array($this->LO_types)) {
			...
			while (list($id_type) = sql_fetch_row($res)) {
				switch ($id_type) {
					case 'scormorg': $this->LO_types[$id_type] = Lang::t('_SCORMSECTIONNAME', 'scorm'); break;
					case 'item': $this->LO_types[$id_type] = Lang::t('_FILE', 'standard'); break;
					default: $this->LO_types[$id_type] = Lang::t('_LONAME_'.$id_type, 'storage'); break;
				}
			}
		}
	...
	function _get_courses_query($type='html', $report_data = NULL, $other='') {
		...
		switch ($show_suspended) {
			case "all": $query_show_suspended = "1"; break;
			case "suspended_only": $query_show_suspended = "u.valid = 0"; break;
			case "active_only": $query_show_suspended = "u.valid = 1"; break;
		}
		...
		switch ($order_by) {
			case 'firstname': $query_order_by = "u.firstname ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'lastname': $query_order_by = "u.lastname ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'email': $query_order_by = "u.email ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'course_code': $query_order_by = "c.code ".$_dir.", u.userid ".$_dir.""; break;
			case 'course_name': $query_order_by = "c.name ".$_dir.", c.code ".$_dir.", u.userid ".$_dir.""; break;
			case 'status': $query_order_by = "cu.status ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'level': $query_order_by = "cu.level ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'date_subscription': $query_order_by = "cu.date_inscr ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'date_first_access': $query_order_by = "cu.date_first_access ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'date_last_access': $query_order_by = "cu.date_last_access ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
			case 'date_complete': $query_order_by = "cu.date_complete ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
		}
	...
	function _printTable_courses(
		...
		$colspanuser = 0;
		$th2[] = $lang->def('_USERNAME'); $colspanuser++;
		if (in_array('_TH_LASTNAME', $cols)) { $th2[] = Lang::t('_LASTNAME', 'standard'); $colspanuser++; }
		if (in_array('_TH_FIRSTNAME', $cols)) { $th2[] = Lang::t('_FIRSTNAME', 'standard'); $colspanuser++; }
		...
		if ($show_classrooms_editions) {
			...
			if (in_array('_TH_CLASSROOM_NAME', $cols)) { $th2[] = Lang::t('_CODE', 'standard'); $colspan_classrooms_editions++; }
			if (in_array('_TH_CLASSROOM_MIN_DATE', $cols)) { $th2[] = Lang::t('_DATE_BEGIN', 'standard'); $colspan_classrooms_editions++; }
...
// Luca 
   ...
   // Luca
	...
	function _get_competences_query($type='html', $report_data = NULL, $other='') {
		...
		foreach ($arr_competences as $cid) { $_head[] = array('style'=>'img-cell', 'value'=>$cinfo[$cid]->langs[$language]['name']); }
				...
				if (isset($filter['flag'])) { //we are checking a competence of type 'flag' --> just check if the score exists and is > 0
					...
					foreach ($filter as $fvalue) {
						...
						if (array_key_exists($id_competence, $ucomps) && $ucomps[$id_competence]>0) {
							...
							$condition = false;
							switch ($_sign) {
								case 0: $condition = $ucomps[$id_competence] < $_value; break;
								case 1: $condition = $ucomps[$id_competence] <= $_value; break;
								case 2: $condition = $ucomps[$id_competence] == $_value; break;
								case 3: $condition = $ucomps[$id_competence] >= $_value; break;
								case 4: $condition = $ucomps[$id_competence] > $_value; break;
							}
							if ($condition) {
								$satisfied++;
							}
	...
	function _get_delay_query($type='html', $report_data = NULL, $other='') {
		...
		if (1==1)//($org_chart_subdivision === 0)
		{
			...
			switch ($show_suspended) {
				...
				case "active_only": $query_show_suspended = " AND u.valid = 1 "; break;
			}
			...
			switch ($order_by) {
				case 'firstname': $query_order_by = "u.firstname ".$_dir.", u.userid ".$_dir; break;
				case 'lastname': $query_order_by = "u.lastname ".$_dir.", u.userid ".$_dir; break;
				case 'email': $query_order_by = "u.email ".$_dir.", u.userid ".$_dir; break;
				case 'status': $query_order_by = "cu.status ".$_dir.", u.userid ".$_dir.", u.lastname ".$_dir; break;
				//case 'level': $query_order_by = "cu.level ".$_dir.", u.userid ".$_dir.", c.code ".$_dir.""; break;
				case 'date_subscription': $query_order_by = "cu.date_inscr ".$_dir.", u.userid ".$_dir.", u.lastname ".$_dir; break;
				case 'date_first_access': $query_order_by = "cu.date_first_access ".$_dir.", u.userid ".$_dir.", u.lastname ".$_dir; break;
				case 'date_last_access': $query_order_by = "cu.date_last_access ".$_dir.", u.userid ".$_dir.", u.lastname ".$_dir; break;
				case 'date_complete': $query_order_by = "cu.date_complete ".$_dir.", u.userid ".$_dir.", u.lastname ".$_dir; break;
			}
					...
					foreach ($this->delay_columns as $delay_row) {
						...
						if (($delay_row['select'] && in_array($index, $showed_cols)) || !$delay_row['select']) {
							switch ($index) {
								...
								case 'email': $line[] = trim($user_info['mail']); break;
								default: $line[] = $user_info[$index]; break;
							}
						}
					}
	...
	function _get_LO_query($type='html', $report_data = NULL, $other='') {
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			switch ($val['key']) {
				...
				case 'lo_type': { if (in_array($val['key'], $showed)) $temp[]=isset($LO_types[$row['objectType']]) ? $LO_types[$row['objectType']] : ""; } break;
